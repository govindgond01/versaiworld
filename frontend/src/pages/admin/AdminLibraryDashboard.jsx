...
const AdminLibraryDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // ✅ Use EXACT SAME APIs as your student management page
      const [studentsRes, statsRes] = await Promise.all([
        // Same API as student listing page
        api.get('/admin/students?studentCategory=library&status=active&limit=7'),
        // Stats API
        api.get('/admin/students/stats')
      ]);

      console.log('📊 Library Dashboard API Responses:', {
        students: studentsRes.data,
        stats: statsRes.data
      });

      // Extract data based on YOUR controller response
      const students = studentsRes.data?.students || [];
      const stats = statsRes.data || {};
      
      // Filter library students from stats
      ...
      // Calculate totals from fetched students
      const totalStudents = students.length;
      const activeStudents = students.filter(s => s.status === 'active').length;
      
      // Get expiring students from the same list (if they have expiryDate)
      ...
      // Calculate fees from students data
