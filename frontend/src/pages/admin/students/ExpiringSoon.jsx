...
const ExpiringSoon = () => {
  ...
  const fetchExpiringStudents = async () => {
    try {
      setLoading(true);
      const res = await api.get('/admin/students/expiring-soon');
      if (res.data.success) {
      ...
      console.error('Fetch expiring students error:', error);
  ...
  const handleRenew = async (id, name) => {
    ...
    try {
      await api.patch(`/admin/students/${id}/renew`, { duration: '1_month' });
      alert('Renewed!');
              ...
              <div key={s._id} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md transition">
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-3 pt-3 border-t">
                    ...
                    <button onClick={() => navigate(`/admin-dashboard/students/${s._id}`)} className="flex items-center gap-1.5 px-4 py-2 border border-gray-600 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 ml-auto">
